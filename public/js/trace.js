var splunk = {
    ...
    get : function(eid){
        for(var i in this.cls){
            ...
            return this.url+this.cls[i].f+(eid.attr('title').split(':')[1]);
        }
    }
};
...
$(document).ready(function(){
    ...
    var render_ = function(data){
        $('#main').html(ejs.render($('#mainTemplate').html(), {
            ...
            fn: {
                ...
                form : ejs.compile($('#formTemplate').html()),
                eids : ejs.compile($('#eidsTemplate').html())
            }
        }));
    ...
    var init_ = function(){
        var hashed = window.location.hash.slice(1) || '/trace/';
        $.ajax({
            url : hashed,
